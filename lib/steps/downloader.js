...
module.exports = function (opts) {
  ...
  return function downloader(obj, cb) {
    ...
    obj.downloadAttempt = 1;

    exponentialBackoff.failAfter(opts.failAfter || 12);
    exponentialBackoff.on('fail', function () {
      var message = "impossible to download " + obj.sourceUrl;
      debug(obj.downloadAttempt + " " + message);
      cb(message);
    });

    debug('Downloading: ' + obj.sourceUrl + " ...");
    ...
    exponentialBackoff.on('ready', function (number, delay) {
      ...
      timeout = function () {
        if (obj.stream) {
          ...
          debug((obj.downloadAttempt++) + " backoff for " + obj.sourceUrl + " because of timeout");
      ...
      error = function (err) {
        ...
        if (!completed) {
          ...
          // we can backoff
          debug((obj.downloadAttempt++) + " backoff for " + obj.sourceUrl + " because of " + err.code);
          exponentialBackoff.backoff();
        } else {
          if (obj.stream) {
            debug((obj.downloadAttempt++) + "the source errored, closing downstream for " + obj.sourceUrl);
      ...
      response = function (res) {
        debug((obj.downloadAttempt++) + " status code " + res.statusCode + " for " + obj.sourceUrl);
        completed = true;
